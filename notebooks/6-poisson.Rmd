...
library(maxLik) ## to estimate models by ourselvs
...
```{r}
m1 <- glm(LOS_LT14 ~ I(DOCH/1000) + factor(KLM) + factor(WOJ), 
         data = df,
         family = poisson())
summary(m1)
```
...
```{r}
dispersiontest(m1, trafo = 1) ## quasipoisson / negative-binom1
dispersiontest(m1, trafo = 2) ## negative-binom 
```
